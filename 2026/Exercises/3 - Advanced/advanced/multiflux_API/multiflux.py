...
def setInputDatabyMASS( element, fluxspectrum, runlabel  ):
    ...
    ip.appendschedule(5*pf.util.MIN_TO_SEC(), 1.1E14)
    return ip
...
runname1 = "run1"
# load first spectrum 
flux1 = readfluxfile( "fluxes.1" )

# set input
input1 = setInputDatabyMASS( "W",  flux1, runname1  )

# run FISPACT 
output1 = runFISPACT(input1, nucleardata)
pf.io.to_file(output1, m, "{}.json".format(runname1) )

# to perform sequential calculations
#   - load second flux
#   - extract nuclides from the first output
#   - set a new inputdata
#   - run fispact

pf.finalise(m)
pf.io.to_file(m, "multiflux.log")